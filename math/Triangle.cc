...
Triangle::Triangle(Vector3 a, Vector3 b, Vector3 c): a(a), b(b), c(c) {
    this->getNormal(this->a, this->b, this->c, this->normal);
}

Triangle::Triangle(Vector3 a, Vector3 b, Vector3 c, Vector3 normal): a(a), b(b), c(c), normal(normal) {}
...
Triangle& Triangle::copy(const Triangle& t) {
...
void Triangle::closestPointToPoint(const Vector3& p, Vector3& target) const {
    ...
    const auto a = this->a, b = this->b, c = this->c;
    float v, w;
